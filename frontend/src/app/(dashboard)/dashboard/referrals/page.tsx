...
export default function ReferralsPage() {
  ...
  const referralLink = `https://virail.com?ref=${userData.referralCode}`;
  ...
  const handleShare = () => {
    if (navigator.share) {
        navigator.share({
            ...
            text: 'Get $50 off your first month of AI video generation!',
            url: referralLink
        }).catch(() => handleCopy());
    } else {
        handleCopy();
  ...
  const handleApplyAffiliate = () => {
    setApplied(true);
    alert("Application for Affiliate Status submitted! We will review your profile and contact you via email.");
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-8 mb-12">
        <Card className="lg:col-span-2 bg-gradient-to-br from-blue-600 to-purple-700 p-10 rounded-[3rem] relative overflow-hidden border-none shadow-2xl shadow-blue-500/20">
            ...
            <div className="relative z-10">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                    ...
                    <Button 
                        onClick={handleShare}
                        className="bg-white text-blue-600 hover:bg-blue-50 font-black uppercase tracking-widest rounded-2xl px-8 h-14"
                    >
            ...
            <Card className="bg-slate-900/50 border-slate-800 p-8 rounded-[2.5rem] glass-panel">
                ...
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <h3 className="text-3xl font-black text-white">{userData.stats.clicks}</h3>
                    </div>
                    <div>
                        <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Signups</p>
                        <h3 className="text-3xl font-black text-white">{userData.stats.signups}</h3>
                    </div>
                </div>
                <div className="mt-8 pt-8 border-t border-slate-800">
                    <p className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-1">Credits Earned</p>
                    <h3 className="text-3xl font-black text-emerald-500">${userData.stats.earned}.00</h3>
                </div>
            </Card>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        <Card className="bg-slate-900/50 border-slate-800 rounded-[2.5rem] overflow-hidden glass-panel">
            ...
            <div className="p-8">
                {userData.recentReferrals.length === 0 ? (
        ...
        <Card className="bg-slate-900/50 border-slate-800 p-8 rounded-[2.5rem] glass-panel flex flex-col justify-between border-dashed">
            <div>
                ...
                <h3 className="text-xl font-bold text-white mb-2 italic uppercase">Join the Affiliate Program</h3>
            ...
            <Button 
                variant="outline" 
                className={`w-full mt-10 border-slate-800 transition-all rounded-xl h-12 uppercase font-black text-[10px] tracking-widest ${
                    applied ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/50" : "hover:bg-emerald-500/5 hover:text-emerald-500 hover:border-emerald-500/30"
                }`}
                onClick={handleApplyAffiliate}
                disabled={applied}
            >
                {applied ? "Application Pending" : "Apply for Affiliate Status"}
            </Button>
        </Card>
      </div>
    </div>
