...
def prompt_input() -> dict:
    ...
    console.print(Panel(
        "[bold cyan]AI Video Shorts Creator[/bold cyan]\n"
        "[dim]Powered by Whisper · CLIP · YOLO · FFmpeg[/dim]",
        border_style="cyan",
    ))
    ...
    while True:
        ...
        console.print(f"[dim]  Resolved: {video_path}[/dim]")
...
def main():
    ...
    if not results:
        console.print("[bold red]No shorts were generated.[/bold red]")
        sys.exit(1)

    console.print(f"\n[bold green]🎉 All done! {len(results)} short(s) saved.[/bold green]")
